...
public class BoardController {
    ...
    public String writeOk(
            @RequestParam Map<String, MultipartFile> files  // 첨부파일들 < name, file> 로 받아냄
            , @Valid Post post
        ...
        if (result.hasErrors()) {
            ...
            // addAttribute
            //    request parameters로 값을 전달.  redirect URL 에 query string 으로 값이 담김
...
//        model.addAttribute("list", boardService.list());
    ...
    public String updateOk(
            @RequestParam Map<String, MultipartFile> files // 새로 추가될 파일
            ,@Valid Post post
            ,BindingResult result
            ,Long [] delfile // 삭제될 파일
            ,Model model
