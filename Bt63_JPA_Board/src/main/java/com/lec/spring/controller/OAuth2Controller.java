...
public class OAuth2Controller {
    ...
    public User registerKakaoUser(KakaoProfile profile){
       ...
       """.formatted(username, name, password, provider, providerId));
        ...
        if (user == null) { // 신규회원인 경우에만 회원 가입 진행
            ...
            if (cnt > 0) {
                ...
                user = userService.findByUsername(username); // id, regdate 를 다시 읽어옴!
            } else {
    ...
    public void loginKakaoUSer(User kakaoUser) {
        UsernamePasswordAuthenticationToken authenticationToken
                = new UsernamePasswordAuthenticationToken(kakaoUser.getUsername(), oauth2Password); // username 과 password
