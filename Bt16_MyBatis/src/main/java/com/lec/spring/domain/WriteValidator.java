...
public class WriteValidator implements Validator {
    ...
    // Validator 가 제공된 class 의 인스턴스 (clazz) 의 유효성 검사를 할 수 있나?
    @Override
    ...
    @Override
    public void validate(Object target, Errors errors) { // support 에서 '참' 으로 호출되면 validate 로 온다.
        System.out.println("validate() 호출");
        Write write = (Write) target;

        String name = write.getName();
        if(name == null || name.trim().isEmpty()) {
            errors.rejectValue("name", "name 은 필수입니다."); // name 필드에 대한 바인딩 오류 정의
        }

        // id 값 음수 못넘어오게 validator 지정
        if(write.getId() < 0){
            errors.rejectValue("id", "id 값은 0 이상이여야 합니다.");
            write.setId(0);
        }
    }
}
