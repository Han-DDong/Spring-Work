...
public class AttachmentController {
    ...
    public ResponseEntity<?> download(Long id) {
        if (id == null) {
            ...
            if (file == null) {
                return new ResponseEntity<>(null, HttpStatus.NOT_FOUND); // 404 에러
            }
        ...
        try {
            // 파일 유형(MIME type) 추출
            String mimeType = Files.probeContentType(Paths.get(path)); // ex) "image/png" 이런식으로 리턴해줌

            // 파일 유형이 지정되지 않은 경우
            if (mimeType == null) {
                mimeType = "application/octet-stream"; // 일련의 byte 스트림 타입의 기본 마임 타입
                                                        // 유형이 알려지지 않은경우 지정
            }
            ...
            return new ResponseEntity<>(null, null, HttpStatus.CONFLICT); // 409 에러 (파일없음)
        }
    }
} // end Attachment Controller
