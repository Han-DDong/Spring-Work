...
@Configuration
public class Config04 { // @Configuration 이 붙으면 bean객체로 생성됨

    public Config04() {
        System.out.println("Config03() 생성");
    }

    @Bean // 리턴타입의 bean 객체 생성 (기본적으로 bean 의 이름은 메소드 이름으로 설정됨)
    public Score score1() {// Score 타입의 bean 생성, bean 의 이름은 score1
        return new Score(100, 80, 76, "좋아요");
    }

    @Bean(name="Han") // "Han" 이라는 이름의 Score 타입 bean 생성
    public Score score2() {
        return new Score(24, 42, 64, "점수아님");
    }

    @Bean
    public Student stu1() {  // Student 타입의 bean 생성, bean 의 이름은 stu1
        return new Student("한정우", 29, score1());
    }
}
