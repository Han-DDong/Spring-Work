...
public class RtController1 {
    ...
    */
    ...
    public HttpBinModel apiTest04(){
        ...
        // getForObject 나 getForEntity 둘 중 어느것을 사용해도 내부에서는
        // jackson 이 동작한다.
    }
    ...
    public String apiTest05(){
        ...
        params.add("id", "ironman");
        params.add("pw", "1111");

        URI uri = UriComponentsBuilder
                .fromUriString(HTTPBIN_URL)
              //  .path("/get?name=Han&age=29") // 404 에러 (path 에 쿼리스트링 넣으면 안됨 경로만 넣어야함)
                .path("/get")
                ...
                .queryParam("nick", "기아").encode()

                .queryParam("family", List.of("Han", "Kim"))
...
//                .contentType(MediaType.APPLICATION_JSON)
    ...
    public String apiTest10(){
        ...
        User user = new User(29, "한정우");


        // ⬇ 요청의 Content-type 이 x-www-form-urlencoded 방식으로 전달
        //  result = rt.postForObject(uri, null, String.class);

        // ⬇ 요청의 Content-type application/json 방식으로 전달
...
//                .header("Content-Type", "application/json")
    ...
    // exchange 사용
    // exchange(String url, HttpMethod method, HttpEntity<?> requestEntity, Class<T> responseType, Object... uriVariables)
    // post 요청 body 에 name-value 쌍으로 요청하기 - x-www-form-urlencoded 방식
