...
public class HomeController2 {
   ...
   """,
    ...
    public HttpEntity<String> http02() throws UnsupportedEncodingException {
      ...
      //  headers.setContentType(MediaType.TEXT_PLAIN); // MIME type = text/plain 으로 response 됨

        headers.add("Content-type", "text/plain;charset=UTF-8");
        headers.add("user-secret", "xxxx");
       // headers.add("my-team is null null", "널널"); // 에러 (headerValue 값으로 한글 사용 못함 사용하려면
                                                        // ⬇ 인코딩 해줘야함
        headers.add("my-team is null null", URLEncoder.encode("널널", "utf-8"));
    ...
    // 일반적으로는 response 는 ResponseEntity 사용
    ...
    public ResponseEntity<HomeController.Product> http04() {
        HomeController.Product product = new HomeController.Product(10, "한정우", false);
    ...
    public ResponseEntity<?> http06() {
        ...
        // new ResponseEntity<>(null, HttpStatus.NOT_FOUND);  // 404 에러
        // new ResponseEntity<>(null, HttpStatus.CONFLICT);  // 409 에러
