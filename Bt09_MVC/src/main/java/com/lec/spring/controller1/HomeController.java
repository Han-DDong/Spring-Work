...
*/
    ...
    @RequestMapping("/") // "/" 라는 url로 request 가 들어오면 아래 메소드가 처리(handle) 한다.
    public String now(Model model) {  // Model : 데이터를 실어나르는 객체, Business logic 처리결과를 담아 View 까지 전달함!
        System.out.println("now() 호출됐음"); // request 들어올때마다 호출되는것을 확인.
...
*/
    ...
    //    그런경우 Run (control + R) 으로 서버재가동

    @RequestMapping("/aaa") // ⬅️ /aaa 로 request 들어오면 처리
    ...
    public void aaabbbccc() {
        // templates/views/ + "aaa/bbb/ccc" + ".html" 을 request 한다
    }
    ...
    public void httpServletRequest(HttpServletRequest request, Model model) {  // 매개변수 순서 관계없음.
    ...
    public List<Integer> ddd() {
        return List.of(10, 20, 30); // ➡️ JSON array 로 변환해서 리턴해줌
    }
    ...
    @RequestMapping("/fff")
    @ResponseBody // Java POJO(평범한 자바 객체) object => JSON object 로 변환됨
    public Product fff() {
        return new Product(10, "자전거", false); // 필드의 결과가 아니고 프로퍼티의 결과.
    }
    ...
    @GetMapping("/get") // 'GET' 방식만 동작함
    @ResponseBody
    public String get(HttpServletRequest request) {
        return U.requestInfo(request);
    }

    @PostMapping("/post") // 'POST' 방식만 동작함
    @ResponseBody
    public String post(HttpServletRequest request) {
        return U.requestInfo(request);
    }

    @PutMapping("/put") // 'PUT' 방식만 동작함
    @ResponseBody
    public String put(HttpServletRequest request) {
        return U.requestInfo(request);
    }

    @PatchMapping("/patch") // 'PATCH' 방식만 동작함
    @ResponseBody
    public String patch(HttpServletRequest request) {
        return U.requestInfo(request);
    }

    @DeleteMapping("/delete") // 'DELETE' 방식만 동작함
    @ResponseBody
    ...
    // 동일 url 에 대한 요청이라도 서로 다른 request method 에 따라 '다른 동작' 을 수행 하려면?
    @GetMapping("/action")
    @ResponseBody
